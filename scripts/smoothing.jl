...
R = F0(0.1)^2*I
H = I
σ = F0(0.1) # 0.3, 0.1 better visual
a = σ^2*I



l = 8*[2,5,5] # not equidistant
...
#    @show θ1, θ2 = cholesky(Symmetric(Γ))\μ0
    ...
    if SAVE
        for i in eachindex(X.x)
            ...
            if PLOT && iter == iters[end]
                p1 = surface(out; shading=false, show_axis=false, colormap = :deep)
        #    p2 = surface(out; shading=false, show_axis=false, colormap = :deep))
                scale!(p1, 1.0, 1.0, 7.5)
                FileIO.save(joinpath(@__DIR__, "..", "output", "surf$iter-$i.png"), p1)
            end
        end
    end
end
